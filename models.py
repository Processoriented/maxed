...
class ForceField(models.Model):
    ...
    def hasRef(self):
        return (self.fieldref_set.count() > 0)

    def refs(self):
        return self.fieldref_set.all()
...
class DataSetObj(models.Model):
    ...
    def allfields(self):
        return self.datasetfield_set.all().order_by('forcefield__label','forcefield__name')
...
class FilterGroup(models.Model):
    datasetobj = models.ForeignKey(DataSetObj, on_delete=models.CASCADE)
    operator = models.CharField(max_length=10, choices=GOPERATORS, default='AND')
...
class DataSetFilter(models.Model):
    group = models.ForeignKey(FilterGroup, on_delete=models.CASCADE, null=True, blank=True)
